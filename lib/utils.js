...
/** Cek perintah ada di PATH (sync, cepat). Aman dari command injection. */
function hasCommand(cmd) {
  // Whitelist karakter — cegah shell metachar masuk
  if (!cmd || typeof cmd !== "string" || !/^[a-zA-Z0-9._\-]{1,64}$/.test(cmd)) {
    return false;
  }
  try {
    const { spawnSync } = require("child_process");
    const which = process.platform === "win32" ? "where" : "which";
    // Pakai array args, NO shell — command injection-proof
    const r = spawnSync(which, [cmd], {
      stdio: "ignore", timeout: 2000, shell: false,
    });
    return r.status === 0;
