...
class Controller extends EventEmitter {
  ...
  bootstrap() {
    ...
    this.antiDdos  = new AntiDdos({ logger: this.logger });
    this.lockdown  = new LockdownManager({ logger: this.logger });
    this.firewall  = new Firewall({
      cfg: this.cfg, logger: this.logger,
      security: this.security, antiDdos: this.antiDdos,
      lockdown: this.lockdown,
    });

    // Forward log entries supaya consumer (GUI) bisa subscribe
    this.logger.on("entry", (e) => this.emit("log", e));
    this.lockdown.on("level", (entry) => this.emit("lockdown", entry));
    this.antiDdos.on("attack-start", (info) => this.emit("ddos", { type: "attack-start", ...info }));
    this.antiDdos.on("attack-end",   (info) => this.emit("ddos", { type: "attack-end", ...info }));
  ...
  async checkPreflight() {
    ...
    // Token tunnel — kalau provider=ngrok tapi token kosong, JANGAN throw error.
    // Fallback ke 'none' (local-only). User bisa setup token via GUI setup wizard
    // atau edit .env kapan saja.
    if (this.cfg.TUNNEL_PROVIDER === "ngrok") {
      const tk = (this.cfg.NGROK_AUTHTOKEN || "").trim();
      if (!tk || tk.includes("isi_token")) {
        this.logger.uncommon(
          "NGROK_AUTHTOKEN kosong → fallback ke local-only (tanpa tunnel). " +
          "Isi NGROK_AUTHTOKEN di .env untuk dapat URL publik."
        );
        this.cfg.TUNNEL_PROVIDER = "none";
      }
    }
  ...
  async shutdown(reason = "shutdown") {
    ...
    try { this.antiDdos?.shutdown(); } catch (_) {}
  ...
  snapshot() {
    return {
      ...
      host      : getHostInfo(),
      lockdown  : this.lockdown?.status() || null,
      ddos      : this.antiDdos?.stats() || null,
    };
  }
