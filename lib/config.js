...
/** Mini parser .env — tidak butuh dotenv. Anti prototype pollution. */
function parseEnv(content) {
  // Object.create(null) cegah __proto__ chain pollution
  const out = Object.create(null);
  if (!content || typeof content !== "string") return out;
  if (content.length > 1024 * 1024) {
    // .env > 1MB jelas anomali
    throw new Error(".env terlalu besar (>1MB)");
  }
  ...
  for (let raw of lines) {
    ...
    // Validasi key: hanya ALPHA_NUMERIC + underscore. Tolak proto pollution.
    ...
    // Cap value size — env var > 8 KB tidak masuk akal
...
function applyEnv(parsed, override = false) {
  ...
  for (const k of Object.keys(parsed)) {
    // Skip dangerous prototype keys (defense in depth — parser sudah filter, tapi double-check)
    if (k === "__proto__" || k === "constructor" || k === "prototype") continue;
    if (!/^[A-Za-z_][A-Za-z0-9_]{0,127}$/.test(k)) continue;
    if (override || process.env[k] === undefined) {
      process.env[k] = String(parsed[k] ?? "");
    }
  }
}
...
/** Baca .env + apply ke process.env. Kalau .env tidak ada tapi .env.example
 *  ada DAN tidak placeholder, auto-create .env dari template. */
function loadEnv() {
  // Auto-create .env dari .env.example bila missing (hindari error "belum diisi"
  // saat user lupa rename).
  if (!fs.existsSync(ENV_FILE) && fs.existsSync(ENV_SAMPLE)) {
    try {
      const sampleRaw = safeRead(ENV_SAMPLE);
      // Hanya auto-copy kalau .env.example sudah tidak punya placeholder "isi_token"
      // (artinya user sudah edit .env.example dengan token valid)
      if (sampleRaw && !sampleRaw.includes("isi_token") && !sampleRaw.includes("isi_password")) {
        safeWrite(ENV_FILE, sampleRaw);
        // Log ke stderr biar user lihat bahwa .env baru saja dibuat
        try { process.stderr.write("\n[topzone] .env dibuat otomatis dari .env.example\n"); } catch (_) {}
      }
    } catch (_) {}
  }
