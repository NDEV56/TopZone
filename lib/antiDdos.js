...
class AntiDdos extends EventEmitter {
  ...
  attachSlowGuard(socket, ip) {
    ...
    let bodyLimit = null;
    const onHeaders = () => {
      // Headers sudah komplet → mulai timer body
    ...
    // fallback: di Node http server, tidak ada event 'headersComplete' di socket
    // tapi 'request' event dipanggil setelah headers ada — caller pakai cancel manual.
